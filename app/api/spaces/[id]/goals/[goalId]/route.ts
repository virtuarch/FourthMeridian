/**
 * PATCH  /api/spaces/[id]/goals/[goalId]  — update a goal (OWNER/ADMIN only)
 * DELETE /api/spaces/[id]/goals/[goalId]  — soft-delete (move to trash) or permanently purge
 *
...
export const PATCH = withApiHandler(async (
  ...
  const existing = await db.spaceGoal.findUnique({
    where: { id: goalId },
  });
  ...
  const updated = await db.spaceGoal.update({
    where: { id: goalId },
    data: {
...
}, "PATCH /api/spaces/[id]/goals/[goalId]");
...
export const DELETE = withApiHandler(async (
  ...
  if (permanent) {
    ...
    await db.spaceGoal.delete({ where: { id: goalId } });
  } else {
    // Soft-delete: move to trash
    await db.spaceGoal.update({
      where: { id: goalId },
...
}, "DELETE /api/spaces/[id]/goals/[goalId]");
