...
- **SCROLL-1 anti-pattern (do not do this).** Putting `flex flex-col` + a scroll body *directly inside* `<GlassPanel>` — and relying on the `flex flex-col`/`max-h` on the `GlassPanel` element to lay them out — is **unsafe and clips**. The intervening `relative z-10` wrapper is not a flex container and has no height, so neither the `flex` nor the cap reaches the children; `flex-1`/`h-full` on the body resolve against an unbounded block and never scroll. **Correct form:** the scroll body lives inside an *inner flex column* (`flex flex-col`) that carries its **own definite `dvh` `max-height`** and `min-h-0`; the body is `flex-1 min-h-0 overflow-y-auto` within it. (Raw-div panels — `<div>` with `flex flex-col max-h-[…dvh]` + an `overflow-y-auto` body — are exempt: without the `GlassPanel` wrapper the chain is direct, and `overflow:auto` zeroes the flex auto-minimum.) This applies to hand-rolled `GlassPanel` modals *and* to the `GlassModal` (widgets) shell.
...
**Sweep result (SCROLL-1 family).** A follow-up sweep found the *same* root cause in two non-primitive surfaces: the `GlassModal` (widgets) shell — which still carried the pre-fix `h-full` inner (affecting `TimelineModal` + KPI/Perspective detail modals) — and `AccountModal`, which placed its header/tabs/bodies/footer directly inside `<GlassPanel>` with `flex flex-col` on the panel (the anti-pattern in §6). Both were repaired with the *same* inner-definite-`dvh` structure, without migrating either to `OverlaySurface` (fixes **F1** = `GlassModal`, **F2** = `AccountModal`). Raw-`<div>` modals (Investments Activity, Debt All-Transactions, admin/security, the drawers) do **not** share this exact defect — they bypass `GlassPanel`, so their `overflow-y-auto` bodies bound directly — and are handled under the ordinary Phase 5/6/7 migrations, not as scroll bugfixes.
...
**Mandated lock technique — fixed-position body (runtime-proven).** The app scrolls on the **window** (`document.scrollingElement === documentElement`); the shell body is `min-h-full flex flex-col` under `html { height: 100% }`. On that layout, `document.body.style.overflow = "hidden"` is **inert** — measured live: with it applied, `window.scrollTo(900)` still scrolled the page and `maxScroll` was unchanged. It neither locks the background nor preserves position, so an overflow-toggle "lock" does nothing and the page still jumps toward the top when a portalled modal opens. Therefore:

- **Never toggle `body { overflow }` for scroll-locking.** Use the shared **`useBodyScrollLock(active)`** hook (`components/atlas/useBodyScrollLock.ts`), which uses the **fixed-position body technique**: capture `window.scrollY`, then pin `body { position: fixed; top: -scrollY; left: 0; right: 0; width: 100% }` (collapses the document to the viewport → `maxScroll → 0`, truly locked, while the `top` offset keeps content visually in place); on release, clear those styles and `window.scrollTo(0, scrollY)` to land back exactly. It also pads `padding-right` by the removed scrollbar's width to avoid a horizontal content shift. This is the iOS-Safari-correct approach (where `overflow: hidden` on the body does not lock).
- **Capture pre-jump via a layout effect.** The lock engages in `useLayoutEffect` (client) so `scrollY` is read and the body pinned *before* the browser's open-time scroll shift — capturing the pre-jump position, not the post-jump one.
- **Reference-counted / nest-safe:** with several overlays open at once, only the first lock captures the position + pins the body, and only the last release restores it.
- Every modal shell — `OverlaySurface` (all migrated modals), `GlassModal` (adapter over the primitive), `BriefModal`, and full-screen instruments like `DebtPayoffSection` — locks through this one hook. Per-component `scrollY` save/restore workarounds are removed once the shell uses the hook (the former `SpaceDashboard` payoff-fullscreen workaround was retired this way).
...
*Implemented & runtime-verified (SCROLL-2): fixed-position `useBodyScrollLock` (layout-effect capture, scrollbar-gutter compensation, nest-safe) applied to `OverlaySurface`, `GlassModal`, `BriefModal`, `DebtPayoffSection`; `preventScroll` on focus calls; `SpaceDashboard` workaround retired. Verified live on the running app: open pins at the exact `scrollY` with the background locked and no horizontal shift, the modal body still scrolls internally, and close restores the exact position.*
