...
| Class | Meaning | Items |
|---|---|---|
| **A** | Must complete before v2.5 closure | Atlas palette-ratchet fence expansion (exit criterion 1). *V25-CLOSE-1 closed ledger reconciliation, prototype containment, test-discovery boundary, archive removal and `.gitignore` ordering; V25-CLOSE-1A greened the CI lint gate.* |
