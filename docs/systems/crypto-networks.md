...
| Ethereum | `CURRENT_POSITION_SUPPORTED` | Native balance via the shared EVM adapter. **The historical blocker is no longer acquisition.** ETH-H1 established that a plain EOA's quantity history is recoverable from state reads alone — `code == "0x" ∧ nonce(a) == nonce(b) ∧ balance(a) == balance(b)` proves the interval empty, which is a deduction rather than a licence and is strictly stronger than the reconciliation BTC and SOL rest on. Proven at zero-wei residual on three public mainnet addresses. Not promoted: no ETH wallet exists for real-wallet acceptance (invariant 45), there is no persistence layer, and contract wallets (Safe, ERC-4337) and 7702-delegated EOAs are refused outright — the promise would hold only for plain EOAs. |
