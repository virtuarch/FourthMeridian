...
### Resolved WITHOUT a `…SpaceData` loader (the boundary is a pure read-model)

- **`WealthResult`** is the canonical, durable **Wealth Workspace boundary** — a single pure client read-model, `computeWealthTimeMachine → WealthResult` (`lib/wealth/wealth-time-machine.ts`), over one **Space-level** shared `Snapshot[]` fetch (the same series Overview `net_worth_chart` and Debt read). As of **SD-5** it is consumed directly by `WealthWorkspace` (`components/space/widgets/wealth/WealthWorkspace.tsx`), which owns the composition, the per-date **display-currency** conversion (`lib/wealth/display-conversion.ts`, `convertWealthSnapshots` — the NetWorthChart per-date model, applied *before* the read-model), the trust envelope, and the Evidence drawer. **`WealthSpaceData` was deliberately NOT created** — Wealth has no multi-read composition graph, so a `loadWealthSpaceData` would only relocate a pure function to the server or duplicate the shared Space-level snapshot fetch under a wealth brand (see `docs/investigations/FOURTH_MERIDIAN_POST_SD3_WORKSPACE_CONTRACT_INVESTIGATION_2026-07-16.md` §1.11). Any future server move for Wealth is a **Space-level snapshot loader** (Overview/Debt-shared), not a wealth-branded one.
...
> **Current reality of the Planned workspaces** (recorded so nothing is over-claimed):
