# Financial Semantic Authorities

**Status:** architecture doctrine (not API docs). **Current through P2-7D** (Canonical Truth-Spine Convergence — Phase 2, formally complete; see `PHASE_2_DOCTRINE.md`).
...
> parallel-authority drift Phase 2 eliminated. When in doubt: **read the authority,
...
- **Authority:** `lib/transactions/flow-predicates.ts`. PURE, zero-import (operates on the `flowType` value as a plain string). Pinned by `flow-predicates.test.ts` (label completeness against the `FlowType` enum) and the doctrine oracle.
- **The predicates:**
  ...
  - `isSpendLedgerFlow = {SPENDING, REFUND}` / `isExcludedFromSpendLedger` — spend-ledger membership (a flow contributes to the spend ledger, as spend or as its reversal). Backs `flow-classifier.ts`'s `isSpendingFlow`/`isExcludedFromSpending` and, transitively, the opportunity-eligibility gate in `lib/ai/intelligence/annotations.ts`. **Distinct from `isCostFlow`** — do not conflate the two "spend" notions (that conflation is what Phase 1 disentangled).
...
- **Inputs:** rows with `flowType`, an amount accessor.
- **Outputs:** `Map<flowType, number>`.
- **Consumers:** `SpaceTransactionsPanel` summary chips + the "By Flow Type" grouped view (both read the same map, §9.8).
- **Must NOT be used for:** economic aggregation with refund netting — it is a raw per-kind sum, not a fold. The clamped economic spend is `clampEconomicSpend`, not a composition of `sumByFlowType` buckets. *(See the residual-debt note in §11 on the one presentation surface that still composes a spend chip from this map.)*
...
- **Authority:** `cash-flow.ts` (`economicTotals`) and `cash-flow-projection.ts` (the rest).
  - `economicTotals(rows, ctx)` — the no-liquidity-context economic shortcut (`{income, spend, refunds, net}`); byte-identical to `perspectiveTotals(aggregateDayFacts(...), "economic")`. Thin projection over `foldEconomicRow` + `clampEconomicSpend`. (Renamed from `aggregateCashFlow` in P2-1A — the old name wrongly implied a second authority.)
...
- **Must NOT be used for:** re-hardcoding the type→tier partition at a consumer. *(See §11 — several balance-reconstruction consumers still duplicate this partition; consistent with `accountTier` today, tracked as cleanup debt, not a rival authority.)*
...
- **Inputs / Outputs:** a `DayFacts` → `LiquidityBreakdown` (`cashIn[]`/`cashOut[]` labeled lines, totals, `netCash`, `unresolved`, `creditCardPurchases`, `internalTransfers`).
...
- **Authority:** `lib/transactions/transfer-evidence.ts` (canonical side); provider adapters (e.g. `plaid-transfer-evidence.ts`, which holds the only payment-app name list) produce it. Pure, no provider strings.
...
- **Authority:** `lib/transactions/transfer-evidence.ts` `deriveTransferDisposition` (stage 2). Provider-neutral; "unknown over incorrect".
...
- **Inputs / Outputs:** a target row + caller-supplied candidate rows → structured relationship facts (ids/roles), never prose.
...
### getCurrentPositions  *(P2-3 · the current-position seam)*
...
- **Consumers:** the AI holdings assembler (`lib/ai/assemblers/holdings.ts`), data export (`lib/export/holdings.ts` + `assemble.ts`), Connections health (`lib/data/investment-accounts.ts`), the future Plan layer. **These are the only production current-position readers, and none reads the legacy `Holding` table.**
- **Visibility:** enforced INSIDE the seam (KD-21a) — current-position DETAIL is always scoped to detail-eligible (FULL) links via the canonical `TRANSACTION_DETAIL_VISIBILITY` predicate. Fails closed to empty. No caller can opt out.
...
- **Must NOT be used for:** the legacy `Holding` table. `Holding` reads are being retired — see §11.

---

# 7 — FX / reporting currency

### convertMoney  ·  ConversionContext  *(P2-7C)*
...
- **Authority:** `lib/money/convert.ts` `convertMoney(money, dateISO, ctx)`, driven by a per-Space `ConversionContext` built from `Space.reportingCurrency` via `lib/money/server-context.ts` (`buildSpaceConversionContext` / `buildSpaceConversionContextById`).
- **Inputs:** a `Money` value, a valuation date, a `ConversionContext`.
- **Outputs:** `ConvertedMoney` (`amount` in reporting currency, `estimated` flag). Missing FX degrades to the native amount + `estimated` — **never dropped, never silently claimed exact** (the MC1 kill-switch: `if (!ctx) return native`, only ever reached if a Space row vanishes mid-request).
...
### reportingBalance  ·  native-balance doctrine  *(P2-7D)*
...
- **Authority:** produced by the accounts assembler (`lib/ai/assemblers/accounts.ts`, via the same `moneyCtx` and valuation date as the section totals) on `AccountSummaryItem.reportingBalance`, with an `estimated` companion.
- **Native-balance doctrine:** `AccountSummaryItem.balance` / `currency` are **detail facts** — for account-specific display only ("Card balance: AED 20,000"). They must **never** be summed, averaged, weighted, ranked, sorted, thresholded, or compared across mixed currencies. Every such operation reads `reportingBalance` (or a per-row `convertMoney` result). *(Reporting-currency invariant, pinned by `debt-strategy-fx.test.ts` and the `.fx.test.ts` suites.)*
...
**Debt strategy / paydown reporting-currency rule (P2-7D).** Debt-strategy math in
`lib/ai/intelligence/annotations.ts` (`monthlyInterestBurden`, `weightedAvgApr`
weighting, snowball ranking, and the surfaced avalanche/snowball candidate
balances) is a **cross-account** computation and therefore uses each account's
**reporting balance** (`AccountSummaryItem.reportingBalance`), never the native
`balance` — per §7. **APR is dimensionless and untouched.** An estimated reporting
balance propagates to `DebtStrategySection.balancesEstimated` so the cross-currency
ranking is never claimed exact.

**Debt payment semantics.** Cash leaving a liquid account toward a liability. The
authority is `classifyLiquidity` → `CASH_OUT / DEBT_PAYMENT` → `DayFacts.byReason.DEBT_PAYMENT`
(source-side liquid legs; the received-on-liability leg is NEUTRAL, so a payment is
counted once). `lib/debt.ts` `totalDebtPaid` / `rollupDebtPaymentsByAccount` are
the **destination-side** view of the *same* fact family (per-liability attribution),
and the AI assembler's `debtPaymentTotal` is the settled-window view — all three are
views of ONE family (`isDebtPayment`), never competing truths; they reconcile via a
future oracle (B-S2), not by picking one.

Each derived quantity below is tagged **AVAILABLE NOW** (a DayFacts fact or trivial
...
- **Must NOT be used for:** a transaction-derived number; `charges − payments` (a different, non-reconciling quantity — see `debtReconciliationResidual`); per-account opening balance (FUTURE PER-ACCOUNT SUPPORT). Backfilled rows hold debt flat (`isEstimated`); `fxMiss` rows are unusable — carry the disclosure.

### G–L. `debtReduction` / `debtCarryover` / `averageCarriedDebt` / `debtFreeStreak` / `debtPaydownVelocity` — AVAILABLE WITH COVERAGE CAVEAT
- **Authority:** balance truth (`max(0, opening − closing)`; closing re-labeled; means/runs/deltas over reliable non-`isEstimated`, non-`fxMiss` snapshot days only).
...
- **Authority:** balance truth reconciled against DayFacts flow facts — a `DebtFacts` projection (**not yet implemented**).
...
Documented as a **contract**; deferred. When built it is a **projection, not a
fourth fold**: every transaction figure reads DayFacts, every balance figure reads
`SpaceSnapshot.debt`, and `residual` is disclosed, never zeroed. Sequenced **after**
the debt-payment reconciliation oracle (B-S2); per-account carryover is gated on
per-account daily balance snapshots (FUTURE DATA REQUIREMENT).
...
than re-deciding a financial truth locally.

- **Transactions assembler** (`lib/ai/assemblers/transactions.ts`) — gates population on `BANKING_POPULATION`, partitions money **exclusively** via the `flow-predicates` predicates (`isNonEconomicResidue`, `isTransfer`, `isDebtPayment`, `isIncome`, `isRefund`, `isCostFlow`), and converts every row via `convertMoney` before summing/sorting. The old private `EXPENSE_FLOWS` set is **removed**. Net = `income + refund − expense − debtPayment`, matching `clampEconomicSpend` doctrine.
- **Accounts assembler** (`lib/ai/assemblers/accounts.ts`) — delegates totals / liquid / net-worth / liability classification to `classifyAccounts` (`account-classifier`); produces `reportingBalance` via the canonical FX seam.
- **Intelligence** (`lib/ai/intelligence/annotations.ts`) — never re-folds raw rows; reads the assembler's already-canonical scalars and `monthlyBreakdown`. Spend-eligibility delegates to `isExcludedFromSpending` (the spend-ledger authority). Debt strategy ranks/weights on `reportingBalance` + `apr`.
...
- **Daily Brief** (`app/api/brief/route.ts`) — projects off canonical assembler outputs (`incomeTotal`, `expenseTotal`, `netWorth`, `netWorthTrend`); does not re-fold transactions.
...
# 11 — Known residual debt (Phase 2 closure audit)

Phase 2 closed with **zero Category-A blockers and zero remaining parallel
semantic authorities.** The following are compatibility / cleanliness / documentation
residues — tracked, non-blocking, and explicitly **not** rival authorities:

- **`SpaceTransactionsPanel.tsx` spend chip** — composes the "Spend" summary chip inline (`SPENDING+FEE+INTEREST − REFUND`, clamped) from the canonical `sumByFlowType` map rather than calling `isCostFlow` + `clampEconomicSpend`. Value-coincident and test-pinned; presentation-tier duplicate to fold back onto the authorities.
- **Account-tier partition duplication** — several balance-reconstruction consumers (`perspective-engine/lenses/liquidity.core.ts`, `asof-completeness.ts`, `accounts-asof.ts`, `isReconstructableCard` copies, snapshot builders) re-hardcode the type→tier / credit-card partition instead of calling `accountTier`. Consistent with it today; cleanup, not divergence.
- **Legacy `Holding` retirement** — the crypto compatibility read bridge (`legacy-crypto-holdings.ts`), transitional dual-writes (`btc-sync.ts`, `sync-current-holdings.ts`), and the dead `getHoldings` (`lib/data/accounts.ts`, no production caller) remain during `Holding`-model retirement. Sanctioned transitional debt.
...
See the Phase 2 Closure Audit and `PHASE_2_DOCTRINE.md` for the full classification.
