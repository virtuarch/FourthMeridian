...
// (REVIEW-3: the sumByFlowType / UNCLASSIFIED_FLOW_KEY aggregation and the
// isSerializedSpendingFlow / isExcludedFromSpendLedger wrappers were deleted —
// zero importers outside this test after TX-3.3. Their membership semantics
// are still pinned below through the surviving sets/predicates.)
...
test('SERIALIZED_SPENDING_FLOWS is exactly {SPENDING, FEE} (narrower than cost)', () => {
  ...
  assert.equal(SERIALIZED_SPENDING_FLOWS.has('INTEREST'), false);
});

test('spend-ledger is exactly {SPENDING, REFUND}', () => {
  for (const ft of ALL) {
