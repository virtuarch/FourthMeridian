...
export async function commitInvestmentImport(input: CommitInput): Promise<CommitResult> {
  ...
  try {
    ...
    await recordSyncIssue({ kind: "INVESTMENT_DATA_PERSISTENCE_FAILED", financialAccountId, detail: { stage: "investment-import-repair", error: err instanceof Error ? err.message : String(err) } }, client);
  }
