 ...
 *
 * Covers chunking/pagination and daily missing-date selection — no DB, no
 * network.
 *
 * V26-PRICE-3 removed the window-resolution sections that lived here. They
 * exercised resolveBackfillWindow and resolveForceBackfillWindows, both deleted
 * with the single-contiguous-interval assumption they encoded. Their behavioural
 * intent — including a regression for the 2026-07-15 front-edge-coverage
 * collapse — now lives in lib/prices/acquisition-plan.core.test.ts, asserted
 * against coverage-driven planning rather than block-edge arithmetic.
 */

import { chunkWindow, selectInstrumentsMissingDate } from "./backfill-core";
...
function main(): void {
  // ── 1. chunkWindow (batched/paginated) ────────────────────────────────────
  console.log("1. chunkWindow");
  {
  ...
  // ── 2. selectInstrumentsMissingDate (daily job list) ──────────────────────
  console.log("2. selectInstrumentsMissingDate");
  {
