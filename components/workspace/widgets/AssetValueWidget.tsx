 ...
 *
 * Data source: WorkspaceDashboardSection.config (JSON field, already in schema).
 ...
 *     → rendered UI
 *
 * The widget is intentionally unaware of which workspace type it lives in.
 * Everything it needs comes from `meta` + `config`.
 *
 * ── Data model contract ───────────────────────────────────────────────────────
 *   FinancialAccount.balance          = source of truth for the current value
 *   WorkspaceDashboardSection.config.accountId     = which account this widget displays
 *   WorkspaceDashboardSection.config.purchasePrice = rendering metadata (gain/loss)
 *   WorkspaceDashboardSection.config.purchaseDate  = rendering metadata
 *   WorkspaceDashboardSection.config.notes         = rendering metadata
 *
 ...
 *   2. Name heuristic   — regex on account.name (fallback for existing workspaces)
 *   3. First type=other — last resort when workspace has one manual asset
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
/**
 * Rendering metadata from WorkspaceDashboardSection.config.
 *
 ...
 *   3. First type=other — fallback when workspace has only one manual asset
 */
export interface AssetValueConfig {
   ...
   * Set via ManageWorkspaceModal asset section settings.
   */
  accountId?:       string;
...
export interface AssetValueWidgetProps {
   ...
   */
  assetType: AssetType;
  /** Parsed from WorkspaceDashboardSection.config */
